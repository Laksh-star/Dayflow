...
- **Ask about this day** submits only the selected day's tasks, captures, and desktop cards when you press **Ask**. With a direct OpenAI provider configured, it uses that provider for the answer; otherwise it falls back to a local evidence summary. Hold the microphone button to transcribe a question, review the transcript in the field, then choose **Ask**. The answer can be spoken with the Mac's system voice.
