...
- A timed capture appears as a clearly marked dashed **Manual** interval on the Day timeline. It is also shown separately as **Manual / offline time** beside observed desktop time, and contributes to **Total accounted time**. It never changes focus or distraction totals. Untimed captures do not appear on the timeline because Dayward has no truthful position for them.
- **Toggl drafts** include timed manual captures as separate entries with a `manual` tag. A Meeting uses the `Meetings` mapping, Personal uses `Personal`, and Offline work uses `Offline work`; add an explicit mapping for any of those sources before exporting. Untimed notes and tasks never export.
