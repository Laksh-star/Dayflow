...
- Wired the existing application-interaction analysis into the active Weekly dashboard:
  ...
  - Changed graph colour classification to use the dominant recorded context by minutes, rather than marking an application as a distraction after a single distraction-labelled interval.
  - Added click-to-inspect application nodes with the captured time split and strongest visible switches behind each graph relationship.
